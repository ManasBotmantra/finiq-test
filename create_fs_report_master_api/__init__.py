"""This module provides an Azure Function to insert or upsert Financial Statement (FS) Report Master records."""
...
from admin_functions import fs_report_master
from common_functions import authorization, config, utils


def main(req: func.HttpRequest, context: func.Context) -> func.HttpResponse:
    """
    Azure Function to create/update Financial Statement Report Master entries.
            ...
            [
                {
                    ...
                    "fs_type": "BS",
                    "fs_node_name": "Assets",
                    "parent_node_name": null, (optional, resolves parent ID)
                    "node_seq": 1,
                    "reporting_node_flag": false,
                    "mapped_coa_node_id": null (optional)
                },
                ...
            ]
            OR
    ...
    api_name = "create_fs_report_master"
    ...
    try:
        ...
        if isinstance(req_body, list):
            ...
            records = req_body.get('records') or req_body.get('fs_report_master') or req_body.get('fs_records')
            if records is None:
                # If neither is found, check if it's a single record dictionary and wrap it in a list
                if all(k in req_body for k in ["company_id", "fs_type", "fs_node_name"]):
                    records = [req_body]
                else:
        ...
        status_code, status_description, error_message, response_body = fs_report_master.f_insert_fs_report_master(
            records=records,
            connection=None
        )
